...
type ConfigExtends map[string]string
...
//Find element specified by searchPath in nested Map ie. '/elem1/subelem2/elem3'
...
//Select and Expand config's extended configs
func (me Map) Select(path string) (selmap Map, err error) {
	...
	for i := 0; i < len(extends); i++ {
		...
		if file == nil || len(file.(string)) == 0 {
			panic("empty value for file in config")
		}
		...
		if es == nil {
			panic(fmt.Sprintf("Can't find service '%s' in '%s'", service, file))
		}
